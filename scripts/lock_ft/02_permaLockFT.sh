...
if [[ $# -eq 0 ]] ; then
    echo -e "\n \033[0;31m Please Supply A Token Amount: ./02_permaLockFT.sh 123 \033[0m \n";
    exit
fi
...
${cli} query utxo \
    --address ${perma_lock_ft_script_address} \
...
if [ "${TXNS}" -eq "0" ]; then
   echo -e "\n \033[0;31m NO UTxOs Found At ${perma_lock_ft_script_address} \033[0m \n";
   exit;
fi
...
perma_lock_ft_script_address_out="${perma_lock_ft_script_address} + ${script_lovelace} + ${token_amt} ${locking_pid}.${locking_tkn}"
...
script_ref_utxo=$(${cli} transaction txid --tx-file ../tmp/perma-lock-ft-reference-utxo.signed)
...
FEE=$(${cli} transaction build \
    ...
    --spending-reference-tx-in-redeemer-file ../data/add-ft-redeemer.json \
